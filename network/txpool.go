...
type TxPool struct {
	...
	pending *TxSortedMap
	/*
		The maxlength of the total pool of transactions and we will prune the oldest transaction when the pool is full.
	*/
	maxLength int
}
...
func (p *TxPool) Add(tx *core.Transaction) {
	// prune the oldest transacation thats sitting in the whole pool
