...
func NewServer(opts ServerOpts) (*Server, error) {
	...
	// Channel being used to communicate between the json rpc server and the
	// node that will process the message.
...
func (s *Server) processStatusMessage(from net.Addr, data *StatusMessage) error {
	...
	if data.CurrentHeight <= s.chain.Height() {
		s.Logger.Log("msg", "cannot sync blockHeing to low", "ourHeight", s.chain.Height(), "theirHeight", data.CurrentHeight, "addr", from)
		return nil
	}
